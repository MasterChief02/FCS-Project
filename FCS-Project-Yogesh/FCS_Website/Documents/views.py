...
class Doctor_Share (ListView):
  def get (self, request):
    ...
    return render (request, "Documents/Templates/Doctor_shares.html", attributes)
  ...
  def post (self, request):
    ...
    print(request.POST)
    document = request.POST.getlist ("filename")
    patient = request.POST.getlist ("Patient")
    print(document)
    print(patient)
    # try:
    # # write code for which table you want to store transaction record into
    #   print("hello")
    # except:
      # attributes = {"title":"Document upload",
      #               "heading": f"Document could not be shared",
      #               "redirect":"/document/share"}
    return render (request, "Common/Templates/message.html")
